...
export function prepare(draft: TItem, field: string): string | number {
  ...
  return typeof item !== 'undefined' && typeof item === 'string' ? (item as string).toLowerCase().replace(' ', '') : item;
};

export function mainSorting(a: string | number, b: string | number, direction: string) {
  if (direction === 'asc') {
    if (a < b) return -1;
    if (a > b) return 1;
  } else {
...
export function sorter(items: TItems, field: string, direction: string) {
  return items.sort(function (a: TItem, b: TItem) {
